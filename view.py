...
tank=pygame.transform.scale(model.tank_image,model.tank.size)
...
def view():
    ...
    player=pygame.transform.rotate(tank,-model.angle)
    if model.changes:
        tank = pygame.transform.scale(model.tank_image, model.tank.size)
    if model.show_image:
        screen.blit(player, model.tank)
        print(player.get_size(), model.tank.size)
    if model.show_rects:
